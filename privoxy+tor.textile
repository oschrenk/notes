...
bc. Jan 14 13:30:58.313 000002f8 Fatal error: can't open configuration file '/opt/local/etc/privoxy/config':  Permission denied

call

bc. $ sudo -u privoxy privoxy

instead.

Now you can use Privoxy as your Proxy. To use it in your favorite terminal, use

bc. $ export http_proxy=http://127.0.0.1:8118/
...
bc. $ sudo vi /opt/local/etc/privoxy/config

and uncomment the line 
 
bc. forward-socks4a   /               127.0.0.1:9050 .

You can stop privoxy by calling

bc. $ sudo killall privoxy

To check if privoxy is running just open "http://config.privoxy.org/":http://config.privoxy.org/ in your browser.
...
bc. $ sudo port install tor
$ sudo port installed | grep tor
$ tor

You get something like

bc. Jan 14 15:38:42.336 [notice] Tor v0.2.1.21. This is experimental software. Do not rely on it for strong anonymity. (Running on Darwin i386)
...
bc. git clone git://git.torproject.org/git/tor
