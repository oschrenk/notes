...
Git will complain if the working copy has uncommitted changes and about conflicts

bc. $ git merge <branch-name> 

To delete a branch (normally not necessary)

bc. $ git branch -d name-of-branch  
...
Author: Johne Doe <john.doe@host.tld>
...
While the history is rewritten, the files might still be in your local repository until they have been garbage collected. You might want to "cleamup":#cleanup your repository.
...
If you publish a repository to github named @username.github.com@, it will autmomatically be used as your github pages website accesible via "username.github.com":http://username.github.com

It support static HTMl content, but also supports "Jekyll":http://github.com/mojombo/jekyll/, a static site generator. For more nfo you can read up on it on "pages.github.com":http://pages.github.com/

Be advised that only a push from your local repisitory does trigger a publish of your homepage, editing the pages via the webinterface does not trigger the publishing service.
...
h3. @Your branch is ahead of 'origin/master' by 11 commits.@
...
My guess is that I accidently called @git push origin master@. In my case I can safely delete the remote branch

bc. $ git remote rm origin

h3. @error: failed to push some refs to 'git@github.com:usernname/project.git@

I tried pusing changes to github when i got
...
Ok. I had no idea. A quick google search answered it. Just do a pull first.
...
And sure enough I remembered that I edited this file when I was at the office.
