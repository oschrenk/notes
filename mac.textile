...
Just a small collection of hints for the mac platform.
...
h3. Change folder in (save) dialogs

Press @Command + Shift + G@ 

h3. Aggregated information about multiple files/directories

Select files and directories and press @Alt + Cmd + I@

h3. Force expanded dialogs
...
bc. $ defaults write -g NSNavPanelExpandedStateForSaveMode -boolean true
$ defaults write -g PMPrintingExpandedStateForPrint -boolean true

h3. Remove Spotight icon from menu bar

bc. $ sudo chmod 600 /System/Library/CoreServices/Search.bundle/Contents/MacOS/Search

Restart the menu bar

bc. killall SystemUIServer

h3. Disable Dashboard

bc. $ defaults write com.apple.dashboard mcx-disabled -boolean yes

h3. Disable Rosetta

To disable Rosetta, simply run the following command in Terminal:

bc. $ sudo sysctl -w kern.exec.archhandler.powerpc=/usr/libexec/oah/RosettaNonGrata

To re-enable Rosetta after it's been disabled, just run the following command in Terminal:

bc. $ sudo sysctl -w kern.exec.archhandler.powerpc=/usr/libexec/oah/translate
...
h3. Save Password when using Cisco VPN
...
h3. Problems decrunching zip archives formed @*.z00?@

# Change all the .z01, .z02, etc. file extensions to .001, .002, etc.
# Change the file extension of the .zip file to .00X (X= the last numeric file extension from the .001, .002, etc. files + 1)
# $ cat filename.00? > filename.zip
# $ unzip filename.zip
...
h4. IMAP - Used Default folder from Mail host instead of Apple's
...
bc. 09.02.10 9. Feb 00:41:46	[0x0-0xc7fc7f].org.videolan.vlc[67847]	[0x4722b8] main interface error: no interface module matched "globalhotkeys,none"

Resetting the configuration worked for me

bc. /Applications/VLC.app/Contents/MacOS/VLC --reset-config

h2. Using multiple accounts

h3. Share any files between users on the same Mac
