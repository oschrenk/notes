...
The <code>@</code> indicates further file attributes. Consult "Apple File Attributes":#apple-file-attributes for more infos.
...
For further security you might want to consider to giveing only your user the right to read the files. 
...
h4. @Could not open a connection to your authentication agent.@
...
* *Batch rename files, replacing _foo_ with _bar_* *@for file in *.*; do mv $file `echo $file | sed 's/bar/foo/g'` ; done@
* *Create missing directory when moving files* @mkdir -p ./some/path/; mv yourfile.txt $_@
* *Download a file with "authorization" cookie* @wget --server-response --continue --load-cookies cookies.txt http://host.com/file.ext@
* *Recursively remove @.svn@ directories* @find . -type d -name '.svn' -print0 | xargs -0 rm -rdf@
