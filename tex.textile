h1. Tex on OSX with Textmate and Skim
...
bc. 	%!TEX TS-program = xelatex
	%!TEX encoding = UTF-8 Unicode

Your editor of choice should recognize it. When not use command line and run @xelatex@

*DON'T* use @%\usepackage[utf8x]{inputenc}@ as xetex already manages the encoding. Whenyou use the package you will get weird errors as xetex does soemthing to the file and again the utf8x package prodcing wrong utf8 character sequences.

h2. Using otf fonts

bc. otfinfo --family /Library/Fonts/GaramondPremrPro.otf

Your tex distribution should come with @otfinfo@ tool which identifies the name of the font family for use with

bc. 	\usepackage{fontspec}
...
# Select Skim as my viewer in the Textmate preferences. @<code>@Bundles > LaTeX > Preferences ...@
...
@<code>@TM_LATEX_MASTER defines the master latex which is called when building the tex from an included file. 

Open project window, press that little "i" button in the drawer. Set up variable

bc. TM_LATEX_MASTER main.tex
...
Tex creates a lot of temporary files that would ony clutter the version control. Create a file called "ignore-these" with the (patterns of) files you want to ignore.
...
bc. svn -R propset svn:ignore . -F ignore-these

To edit these properties later use
bc. svn propedit svn:ignore .