...
Details like translating the address via DNS, opening a TCP connection and so on aside, the webserver (lets say Tomcat directly gets the reqests), matches the URL against the patterns defined in the @web.xml@ to forward the request to the servlet responsible for answering it. The servlet engine spawns a new thread and therfore starts the lifecycle of a servlet.
...
Peope from "OneSwarm":http://oneswarm.cs.washington.edu/ were so kind providing a 1.4.2 version for Snow Leopard
...
"Jave EE 1.2":http://java.sun.com/j2ee/sdk_1.2.1/techdocs/api/index.html
