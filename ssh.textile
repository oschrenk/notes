...
bc. 	Your identification has been saved in ~/.ssh/id_rsa.
	Your public key has been saved in ~/.ssh/id_rsa.pub.

For further security you might want to consider to giving only your user the right to read the files. 

bc. 	# protect the keys from being read
	cd ~/.ssh
	$ chmod 600 id*
	$ chmod 600 known_hosts

h2. Using SSH Agent on OS X

Call @ssh-add@ for every private key.

bc. 	$ ssh-add -K ~/.ssh/id_rsa 

h3. Could not open a connection to your authentication agent
...
bc.. 	SSH_ENV="$HOME/.ssh/environment"
...
h2. Copy SSH key to server

bc. cat ~/.ssh/id_rsa.pub | ssh account@remoteserver.com "cat - >> .ssh/authorized_keys"

h2. Verify SSH Host fingerprint

bc. The authenticity of host 'host.domain (194.77.33.57)' can't be established.
...
bc. $ ssh-keygen -l -f /etc/ssh/ssh_host_rsa_key.pub

On OSX the public key is hiding in @/etc/ssh_host_rsa_key.pub@, so:

bc. $ ssh-keygen -l -f /etc/ssh_host_rsa_key.pub